...
class Simple_STFT_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Define a Hann window for STFT
        window = torch.hann_window(self.frame_len, device=x.device, dtype=x.dtype)

        y = torch.stft(x, n_fft=self.frame_len, hop_length=self.frame_hop,
                       win_length=self.frame_len, window=window,
...
class SeperationBlock_Stateful(nn.Module):
    ...
    def forward(self, x, in_states):
        ...
        :param in_states: (h, c) where:
            - h: [2, N, hidden_size]
            - c: [2, N, hidden_size]
        :return: mask, out_states
        """

        # Properly unpack in_states
        h, c = in_states  # Unpack tuple of hidden and cell states

        # Correctly split into two LSTM layers
        h1_in, c1_in = h[0:1, :, :], c[0:1, :, :]  # First layer state
        h2_in, c2_in = h[1:2, :, :], c[1:2, :, :]  # Second layer state

        # Process through first RNN layer
        x1, (h1, c1) = self.rnn1(x, (h1_in, c1_in))
        x1 = self.drop(x1)

        # Process through second RNN layer
        ...
        # Recombine states for the next step
        h_out = torch.cat((h1, h2), dim=0)  # Combine hidden states
        c_out = torch.cat((c1, c2), dim=0)  # Combine cell states
        out_states = (h_out, c_out)  # Return as a tuple
